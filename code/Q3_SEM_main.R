...
Haiti.SEM_near = psem(
  ...
  lme(Shoot.growth ~ Seagrass.P + Seagrass.N + SLA + Seagrass.C13, 
      random=~1|Reef/Distance, na.action = na.omit, data=Haiti_near),
  
  lme(C.production ~ Seagrass.P + Seagrass.N + SLA + LAI + Seagrass.C13, 
      random=~1|Reef/Distance, na.action = na.omit, data=Haiti_near),
...
Haiti_far$SLA[72] <- NA#Remove outlier #No difference
...
#Variance inflation factors check
vif(lme(C.production ~ Seagrass.P + Seagrass.N + LAI, 
        random=~1|Reef/Distance, na.action = na.omit, data=Haiti_far))
...
#For these plots, the Script has to be run again, but without the transformation of variables!

Haiti_far$Location = as.factor("Haiti_outside")
Haiti_near$Location = as.factor("Haiti_inside")
Bahamas_far$Location = as.factor("Bahamas_outside")
Bahamas_near$Location = as.factor("Bahamas_inside")
all_grouped <- data.frame(rbind(Haiti_far, Haiti_near, Bahamas_far, Bahamas_near))

# Awesome corr plots with density AND boxplots ------------------------------------

names(all_grouped)
corr.plots = ggpairs(all_grouped, columns = c("Shoot.growth","C.production", "Seagrass.P",
                                              "Seagrass.N", "SLA", "LAI", "Seagrass.C13", "Epiphytes.area",
                                              "Location"), aes(color = Location, alpha = 0.5),
               upper = list(continuous = wrap("cor", size = 2.5)),
               lower = list(continuous = wrap("smooth", alpha = 1, size=0.5)))+
  scale_color_manual(values = c("#A1D99B","#238B45","#BCBDDC","#54278F"))+
  scale_fill_manual(values = c("#A1D99B","#A1D99B","#BCBDDC","#54278F"))+
  theme_few()

corr.plots

plot(Epiphytes.area ~ Location, data = all_grouped)
summary(aov(Epiphytes.area ~ Location, data = all_grouped))
