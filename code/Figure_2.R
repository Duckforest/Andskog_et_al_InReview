...
for(isl in 1:2) {
  ...
  mtext("TL (cm)", 1, line = 2.4, cex = mtext.cex)
...
pees.names <- c("Biomass (kg)", "N:P", expression(N~g~day^-1), expression(P~g~day^-1))

#biomass
u = 1

dot <- bb[ ,pees[u] ]
ciu <- bb[ ,pees[u] ] + bb[ ,paste( pees[u], "sd", sep = ".") ]
cil <- bb[ ,pees[u] ] - bb[ ,paste( pees[u], "sd", sep = ".") ]
...
#P excretion - added to same plot
u = 4
...
segments((1 + seps): (13 + seps), cil*10,
         (1 + seps): (13 + seps), ciu*10,
         col = "grey20",lwd = 1.7 )

points( (1 + seps): (13 + seps), dot*10,
        col = "black", pch = 15, cex = 1.7)
points( (1 + seps): (13 + seps), dot*10,
        col = as.character( bb$colores ), pch = 15, cex = 1.6)

axis(4 ,at = seq(0,12,2),tick = TRUE, labels = seq(0,1.2,.2), las = 2, cex.axis = 1.3)
mtext(expression(P~g~day^-1), side = 4, line = 3.5, cex = mtext.cex) 
...
par( mar = c(1,4.8,1,leftsize), mgp <- c(3,1,5) , cex.lab = 1.6,
     cex.axis = 1.4 )
pees <- c("TN", "TP", "Sediment.N", "Sediment.P", "Sediment.C", "Seagrass.N20","Seagrass.P20")
pees.names <- c("TN", "TP","Sediment %N","Sediment %P","Sediment %C", "Amb. SG %N", "Amb. SG %P")
...
dot <- tapply(resp[,pees[u]], resp$Reef, mean, na.rm=T)[r.ord$reef]*40#resp[ ,c("Reef",pees[u]) ]
dot.sd <- tapply(resp[,pees[u]], resp$Reef, sd, na.rm=T)[r.ord$reef]
ciu <- dot+dot.sd
cil <- dot-dot.sd
...
axis(4 ,at = seq(0,400,100),tick = TRUE, labels = seq(0,4,1), las = 2, cex.axis = 1.3)
mtext("TP", side = 4, line = 2.3, cex = mtext.cex) 

###########################

##########################
###### Sediment N and Sediment P #########
...
dot <- tapply(resp[,pees[u]], resp$Reef, mean, na.rm=T)[r.ord$reef]*10#resp[ ,c("Reef",pees[u]) ]
dot.sd <- tapply(resp[,pees[u]]*10, resp$Reef, sd, na.rm=T)[r.ord$reef]
ciu <- dot+dot.sd
cil <- dot-dot.sd
...
points( (1 + seps): (13 + seps), dot,
        col = as.character( bb$colores ), pch = 15, cex = 1.6)
axis(4 ,at = seq(0,.25,.05),tick = TRUE, labels = seq(0,.25/10,.05/10), las = 2, cex.axis = 1.2)
mtext("Sediment %P", side = 4, line = 3.5, cex = mtext.cex) 
...
axis(4 ,at = seq(0,2,.5),tick = TRUE, labels = seq(0,2/10,.5/10), las = 2, cex.axis = 1.3)
mtext("Amb. SG %P", side = 4, line = 3.5, cex = mtext.cex) 
